...
class logger {
public:
	enum level : char { All, Info, Verbose, Warning, Error, Fatal, Count };
...
#define END_TIMER(ID)                                                                                     \
	INFO_LOG("\"%s\" took %f ms",                                                                         \
			 #ID,                                                                                         \
			 std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - ID) \
					 .count() / 1000.f);
